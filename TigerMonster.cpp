...
TigerMonster::TigerMonster(::Game* game)
	: Actor(game)
	, rotationSpeed_(0.1f)
	, rotationAngle_(2.0f)
{
	...
	Texture* texture = new Texture; texture->Load("Assets/Tiger.png");
	std::vector<Mesh*> meshes; LoadAssimpMeshes(meshes, game, "Assets/Tiger.fbx", texture);
	setSpecular(meshes, 30000.0); // we don't want a shiny man :)
	Vector3 startPos = Vector3(1500.00, 500.00, -100.00);
	...
	mMoveComp->SetForwardSpeed(300);

	//new TargetComponent(this); //Ashley Add Target Component for Radar
}

void TigerMonster::UpdateActor(float deltaTime)
{
	FixCollisions();
}

bool TigerMonster::Update(float dt)
{
...
void TigerMonster::FixCollisions() // simply added in by Adam C from madhav code
{
