...
MonsterActor::MonsterActor(Game* game)
	:Actor(game)
	, mMoving(false)
	, angularMovement(1.0)
	, mIsAggro(false)
	...
	, mMonsterMovementSpeed(300)
{
	...
	mMeshComp->SetMesh(game->GetRenderer()->GetMesh("Assets/Monkey.gpmesh"));
...
void MonsterActor::UpdateActor(float deltaTime)
{
	FixCollisions();
...
void MonsterActor::ActorInput(const uint8_t* keys)
{
...
void MonsterActor::MonsterAttackMode() // Function made by Adam C
{
...
void MonsterActor::MonsterShoot()
{
...
void MonsterActor::SetVisible(bool visible)
{
	mMeshComp->SetVisible(visible);
}

void MonsterActor::SetAngularSpeed(float newSpeed)
{
	angularMovement = newSpeed;
}

void MonsterActor::SetAggroState(bool AggroState)
{
	mIsAggro = AggroState;
	mMonsterHostile.SetPaused(false); //Rebecca Morris
	mMonsterHostile.Restart();
}

void MonsterActor::SetHealthState() //Rebecca Morris
{
...
void MonsterActor::FixCollisions() // simply added in by Adam C from madhav code
{
