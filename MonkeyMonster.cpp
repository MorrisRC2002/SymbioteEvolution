...
MonkeyMonster::MonkeyMonster(::Game* game)
	: Actor(game)
	, rotationSpeed_(1.0f)
	, rotationAngle_(2.0f)
{
	...
	Texture* texture = new Texture; texture->Load("Assets/Monkey.png");
	std::vector<Mesh*> meshes; LoadAssimpMeshes(meshes, game, "Assets/Monkey.fbx", texture);
	setSpecular(meshes, 30000.0); // we don't want a shiny man :)
	SetPosition(Vector3(-2100.0f, 2000.0f, -100.0f));
...
void MonkeyMonster::UpdateActor(float deltaTime)
{
	FixCollisions();
}

bool MonkeyMonster::Update(float dt)
{
...
void MonkeyMonster::FixCollisions() // simply added in by Adam C from madhav code
{
